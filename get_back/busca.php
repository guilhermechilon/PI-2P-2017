			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-9 col-sm-9" style="background-image: url(img/wall.jpg);background-attachment: fixed;background-position: center;background-repeat: no-repeat;background-size: cover;height: 100%;">
						...
						<?php 
							$sql = mysqli_query($conn,"SELECT DATE_FORMAT(p.data_post, '%d/%m/%Y %H:%i:%s') as data_post,p.id_post,p.id_usuario,p.post,p.img_post,u.nome,u.foto_perfil,u.foto_perfil FROM postagem as p INNER JOIN usuario as u ON u.id_usuario = p.id_usuario WHERE p.post LIKE '%".$busca."%' OR u.nome LIKE '%".$busca."%' ORDER BY p.data_post DESC LIMIT 10");
						...
						<?php }else{ ?>
							<div class="col-md-8 col-md-offset-3 col-sm-10 col-sm-offset-2">
								<div class="profile-timeline" style="text-align: center;">
									<strong style="font-size: 1.5em;color: white;"><i>Nada foi encontrado com " <?php echo $busca; ?> " em nosso site!!</i></strong>
								</div>
							</div>
						<?php } ?>
					</div>
